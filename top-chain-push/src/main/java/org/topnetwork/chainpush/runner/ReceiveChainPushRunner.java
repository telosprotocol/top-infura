    ...
    public void run_imp() {
        try {
            ...
            while (it.hasNext()) {
                ...
                try {
                    //解析数据
                    TableBlockResp tableBlockResp = TopGrpcClient.xrpc_reply2Stream(xrpc_reply);
                    LOG.info("recv data:", tableBlockResp.toString());
                    sendResult= blockStreamProducer.syncSend(topic, tableBlockResp);
                } catch (Exception e) {
